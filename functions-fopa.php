...
function posts_relacionados($count = 3) {
    ...
    //print "<ul>";
    $query = "
	SELECT DISTINCT {$table_prefix}posts.guid,{$table_prefix}posts.post_title,{$table_prefix}posts.ID
...
//    if(empty($res))
//        $out .= '<p>Nenhuma notícia relacionada foi encontrada.</p>';
    //d($res);
    $c = 1;
    foreach ($res as $post) {
        $last = ($c == $count)?('last'):('');
        ...
        if(has_post_thumbnail()){
            $out .= "
                    <a href='{$post->guid}'>". get_the_post_thumbnail($post->ID,'relacionadas')."</a>
...
function excerpt_relacionadas($length) {return 30;}
